...
/**
   BELLONA set of vars related to loop play.
*/
int nextTrack = 1; //0 index for arrays though.
int numTracks = 2; //placeholder, setup will match to length of "tracks" array
//i get errors with const char* to char* in C++. using const_cast<type*> to remove const from char* when expecting simply char*
char *tracks[] = {
  const_cast<char*>("track001.mp3"), 
  const_cast<char*>("track002.mp3")
  };
  //track names can be whatever is named on the harddrive, e.g. "mysong.mp3"
  //can include hyphens and underscores. e.g. "Longfield-2m_-18dBNorm.mp3"
  //found the usage of this C++ casting here: https://stackoverflow.com/questions/20984220/invalid-conversion-from-const-char-to-char
...
void setup() {
  ...
  //*** BELLONA initialize the mp3 player volume to -6dB
  // push byte[1] into both left and right assuming equal balance.
  MP3player.setVolume(12, 12); // commit new volume -6dB (value is -0.5dB, so 12*-0.5 =-6)
  Serial.print(F("Volume changed to -"));
  Serial.print(12 >> 1, 1);
  Serial.println(F("[dB]"));

  //resize numTracks index to dynamically match our track list array, so we loop ALL named tracks.
  numTracks = sizeof(tracks);
  Serial.print(F("Number of tracks in sketch: "));
  Serial.print(numTracks >> 1, 1); //convert int to byte for printing correctly

  //add a loop that prints off the array of track names, 
  //so when you plug in device you get a list of tracks the sketch is looking for 
}
...
void loop() {
  ...
  //*** BELLONA loop through files

  //play file when done playing based upon number, loop through generically named tracks
//  if (MP3player.isPlaying() != 1) {
//    MP3player.playTrack(nextTrack); //filename must be "track00x.mp3"
//    nextTrack += 1;
//    if (nextTrack > numTracks) {
//      //MP3player.playMP3(const_cast<char*>(four.c_str())); //remove const from char* when expecting simply char*
//      MP3player.playMP3(const_cast<char*>("count4.mp3")); //remove const from that String filename has when expecting simply char*
//      nextTrack = 1;
//    }
//  }

  //play file when done playing based upon String array of filenames
  //filename based upon string array set at top of code and that matches filenames of audio on the SD card
  if (MP3player.isPlaying() != 1) {
    MP3player.playMP3(tracks[nextTrack-1]); //0 index
    //cue next track to play
    nextTrack += 1;
    if (nextTrack > numTracks) {
      nextTrack = 1;
    }
